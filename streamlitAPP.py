...
with dataset:
    ...
    st.markdown('* **VADER Sentiment Analysis:** Using VADER (an open-source package within NLTK) we analyze #NFT tweets for the top 100 projects and extract the Compound Score, Positive Score, Neutral Score, and Negative Score. In the dataset above all Compound Scores below / equal to 0.145 are labeled in red, and all Compound Scores above 0.145 are labeld in green. To calculate the sentimental score of the tweets, VADER scans for known sentimental features, calculates the intesity and polarity of the tweet, then sums up the scores of all the features and normalizes the Compound Score to a range of: (-1 , +1).')
